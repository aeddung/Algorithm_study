...
def solution(n, words):
    answer = []
    tmp = words[0][-1] # 첫 단어 끝 글자
    per_cnt = 1 # 사람
    wor_cnt = 1 # 단어
    spoken = [words[0]] # 이미 말한 단어 리스트
    for word in words[1:]:
        per_cnt += 1
        wor_cnt += 1
        if tmp != word[0]: # 끝 글자와 첫 글자가 다른 경우
            if wor_cnt % n != 0:
                answer = [per_cnt, wor_cnt // n + 1]
            else:
                answer = [per_cnt, wor_cnt // n]
            return answer
        # 단어 중복
        if word in spoken:
            if wor_cnt % n != 0:
                ...
                answer = [per_cnt, wor_cnt // n]
            return answer
        if per_cnt % n == 0: # 사람의 경우 n으로 나누어 떨어지면 순환
            per_cnt = 0
        tmp = word[-1] # 끝 글자
        spoken.append(word)
